...
class Cell:
    def __init__(self, x, y, state=tree):
        ...
        if random.uniform(0,1) < probTree:
            ...
            print("I'm an empty cell")
            self.x = x
            self.y = y
            self.state = empty

    def setState(self, N, E, S, W, t):
        """
        ...
        #print(all_states[t-2][self.x][self.y],all_states[t-1][self.x][self.y],self.state)
        if (self.state == burning and all_states[t-2][self.x][self.y] == burning):
...
for t in range(1,t):
        ...
        for i in range(1,n-1):
                for j in range(1,n-1):                   
                    bar[i][j].setState(bar[i-1][j].state, bar[i][j+1].state, bar[i+1][j].state, bar[i][j-1].state,t)
                    all_states[t][i][j] = bar[i][j].state
...
anim.save('forest_burn2.mp4', writer=writer)
plt.show()
