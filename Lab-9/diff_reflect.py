...
for t in range(1,t):
        ...
        for i in range(m):
            bar[i][n-1].state = bar[i][n-2].state
            bar[i][0].state = bar[i][1].state
            all_states[t][i][n-1] = bar[i][n-1].state
            all_states[t][i][0] = bar[i][0].state

        for j in range(n):
            bar[0][j].state = bar[1][j].state
            # print(bar[0][j].state, bar[1][j].state)
            bar[m-1][j].state = bar[m-2][j].state
...
anim.save('anim_reflecting.mp4', writer=writer)
plt.show()
